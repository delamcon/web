...
def main():
    ...
    def cart():
        if request.method == 'GET':
            info_user = Users.query.filter(Users.id == int(session['id'])).all()[0]  # ищем самого пользователя по айди
            info_orders = Orders.query.filter(Orders.id_of_user == int(session['id'])).all()
    ...
    @app.route('/admin85367/panel/delete_item', methods=['POST', 'GET'])  # страница удаления товара из каталога
    def delete_item():
        if request.method == 'GET':
